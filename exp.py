...
class Simulation:
    ...
    def init_model(self):
        ...
        """
        myc = 1
        y0 = np.zeros(self.parameters["alpha"]+1*self.parameters["alpha_p"]+myc)
    ...
    def get_cells(self):
        ...
        """
        teff = self.state_raw[:, self.parameters["alpha"]:-1]
    ...
    def run_ode(self, hmax = 0.0):
        ...
        # take care at this integration time step for IL2 perturbation
        tcrit = params["time_il2_perturb"] if ("time_il2_perturb" in params) else 0
        state = odeint(model.th_cell_diff, y0, time, args = (mode, params, core), 
                       tcrit = tcrit, hmax = hmax) 
    ...
    def get_il2_max(self):
        ...
        c_il2_ex = np.zeros_like(time)
        
        # add ext. il2 conc. after perturbation time
        for i in range(len(time)):
            t0 = time[i]
            if t0>d["time_il2_perturb"]:
                c_il2_ex[i] = d["c_il2_ex"]*np.exp(-1*(t0-d["time_il2_perturb"]))


        il2 = (c_il2_ex + d["rate_il2"]*il2_producers) / (d["K_il2"]+il2_consumers)
